...
static func update_achievement(achievement: AchievementResource):
	if !achievement.ach_completed:
		achievement.ach_progress = AchievementManager.the_almighty_storage_dictionary["equipment_crafted"]
		if step_one(achievement):
			achievement.ach_current_step = 1
			achievement.ach_1st_description = "Craft"
			achievement.ach_2nd_description = "items."
